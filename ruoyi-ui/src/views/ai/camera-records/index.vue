<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ai:cameraRecords:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['ai:cameraRecords:export']"
        >导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['ai:cameraRecords:query']"
          >查看</el-button>
          <el-button
            type="text"
            icon="Edit"
            @click="handleUpdate(scope.row)"
            v-hasPermi="['ai:cameraRecords:edit']"
          >修改</el-button>
          <el-button
            type="text"
            icon="Delete"
    ...
    <!-- 查看详情对话框 -->
    <el-dialog title="摄像记录详情" v-model="viewOpen" width="80%" append-to-body>
...
<script setup name="CameraRecords">
import { listCameraRecords, getCameraRecords, delCameraRecords, updateCameraRecords } from "@/api/ai/cameraRecords";
...
function getList() {
  ...
  listCameraRecords(queryParams.value).then(response => {
...
function handleUpdate(row) {
  ...
  getCameraRecords(id).then(response => {
...
function submitForm() {
  proxy.$refs["formRef"].validate(valid => {
    if (valid) {
      updateCameraRecords(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除摄像记录编号为"' + delIds + '"的数据项？').then(function() {
    return delCameraRecords(delIds);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('ai/cameraRecords/export', {
    ...queryParams.value
  }, `camera_records_${new Date().getTime()}.xlsx`)
}
